...
class Switch:
    ...
    def connect(self, ip, port):
        ...
        login_match_u = re.compile(b'login:')
    ...
    def send_commands(self, cmd):    
        ...
        time.sleep(3)
        self.output2 = self.sw.read_until(b'#')
        
        self.output += self.output2.decode("ascii")
        print(self.output)

    def grab_info(self):
        
        version_regex = re.compile(r'NXOS:\sversion\s(\d.\.\d\(\d.)')
        serial_regex = re.compile(r'(serial|Serial|SN:)(\snumber\sis)?\s(\w.*)')
        pid_regex = re.compile(r'(Model\snumber|PID:|type)(\sis)?\s(\w{3,4}\-?\w{2,8}\-\w{1,5}(\-\w{1,6})?)')
        part_regex = re.compile(r'(PN:|Part\sNumber|cisco\spart\snumber)(\sis)?\s(\w.*)')
        descr_regex = re.compile(r'([^cable]\stype\sis|Module\stype\sis\s:|DESCR:)(\s?:)?\s\"?(\w.*)(?<!\")')
        hw_ver_regex = re.compile(r'(HWVER:|H/W\sversion|revision)(\sis)?\s(\w.*)')
        ver_id_regex = re.compile(r'(VID:|Part\sRevision|cisco\sversion\sid)(\sis)?\s(\w{1,3})')

        self.version = version_regex.findall(self.output)[0]
        self.my_dict["PID"] = add_to_dict(pid_regex.findall(self.output))
    ...
    def get_fans(self):
        self.send_commands('show inv | i Fan')
        self.send_commands('show inv | i FAN')
...
if __name__ == '__main__':
    ...
    print("Default login is admin/Insieme1234")
    print("Please have samplesbinaryfile.xlsx in the same directory")
    y_n = 'no'

    while y_n == 'no':
        y_n = input("Is the samplesbinaryfile.xlsx in the same directory?? ")
